...
def _vec_ok(inputs, shape, out_dtypes, compute_torch, V):
    ...
    # is contiguous, the element count is a multiple of V (exact 128-bit-aligned
    # (numel/V, V) reshape), AND every output dtype equals the compute dtype. The
    # last rules out bool (gt) and mixed (frexp -> int32) outputs, whose narrow /
    # non-compute element widths can't take the wide vector copy. Those go general.
